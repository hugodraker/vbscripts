...
Sub ConvertTxtToPdf()
    ...
    headerTitle = fso.GetFileName(inputFile)
    ...
    Call BuildCompletePdfWithXref(processedData, headerTitle)
End Sub
...
Sub BuildCompletePdfWithXref(textData, headerTitle)
    ...
    ' OBJ 4: Bold Font (Helvetica-Bold)
    objSegmentStart = Len(fullPdf)
    fullPdf = fullPdf & "4 0 obj" & vbCrLf
    fullPdf = fullPdf & "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>" & vbCrLf
    fullPdf = fullPdf & "endobj" & vbCrLf
    xrefOut(4) = objSegmentStart
    
    ' OBJ 5+: Pages and Content Streams
    pageObjsStart = 4
    ...
    For i = 0 To UBound(consolidatedPages)
        ...
        fullPdf = fullPdf & "/Font << /F1 3 0 R /F2 4 0 R >> >>" & vbCrLf
        fullPdf = fullPdf & "/Contents " & CStr(pageObjsStart + (i * 2) + 2) & " 0 R >>" & vbCrLf
        fullPdf = fullPdf & "endobj" & vbCrLf
        xrefOut(pageObjsStart + (i * 2) + 1) = objSegmentStart
        
        ' Content Stream
        pageContentStr = GenerateTextContentForPage(consolidatedPages(i), i + 1, totalPages, headerTitle)
...
Function GenerateTextContentForPage(pageText, pageNum, pageCount, headerTitle)
    Dim lines, i, yPos, xPos, startY, encodedLine
    Dim contentBuilder, headerStr, footerStr, headerX, footerX
    Dim charWidth9pt
    
    charWidth9pt = 5.2  ' Estimated width per character for 9pt Helvetica-Bold
    contentBuilder = ""
    
    ' 1. Header (Grey 9pt Bold centered at top)
    If Len(headerTitle) > 0 Then
        headerStr = EscapePdfString(headerTitle)
        headerX = Int((pageSizeW - (Len(headerTitle) * charWidth9pt)) / 2)
        If headerX < marginLeft Then headerX = marginLeft
        
        contentBuilder = contentBuilder & "BT" & vbCrLf
        contentBuilder = contentBuilder & "/F2 9 Tf" & vbCrLf  ' /F2 = Helvetica-Bold
        contentBuilder = contentBuilder & "0.5 g" & vbCrLf     ' 50% grey fill
        contentBuilder = contentBuilder & "1 0 0 1 " & CStr(headerX) & " " & CStr(pageSizeH - topMargin + 18) & " Tm" & vbCrLf
        contentBuilder = contentBuilder & "(" & headerStr & ") Tj" & vbCrLf
        contentBuilder = contentBuilder & "ET" & vbCrLf
    End If
    
    ' 2. Footer (Grey 9pt Bold centered at bottom)
    footerStr = "Page " & CStr(pageNum) & " of " & CStr(pageCount)
    footerX = Int((pageSizeW - (Len(footerStr) * charWidth9pt)) / 2)
    
    contentBuilder = contentBuilder & "BT" & vbCrLf
    contentBuilder = contentBuilder & "/F2 9 Tf" & vbCrLf      ' /F2 = Helvetica-Bold
    contentBuilder = contentBuilder & "0.5 g" & vbCrLf         ' 50% grey fill
    contentBuilder = contentBuilder & "1 0 0 1 " & CStr(footerX) & " " & CStr(bottomMargin - 24) & " Tm" & vbCrLf
    contentBuilder = contentBuilder & "(" & EscapePdfString(footerStr) & ") Tj" & vbCrLf
    contentBuilder = contentBuilder & "ET" & vbCrLf
    
    ' 3. Body Text (Black 12pt Regular text)
    lines = Split(pageText, "|NEWLINE|")
    startY = pageSizeH - topMargin
    xPos = marginLeft
    
    contentBuilder = contentBuilder & "BT" & vbCrLf
    contentBuilder = contentBuilder & "/F1 " & fontSize & " Tf" & vbCrLf  ' /F1 = Helvetica Regular
    contentBuilder = contentBuilder & "0.0 0.0 0.0 rg" & vbCrLf         ' Reset color to black
